...
describe("extractThrottle", () => {
  ...
  it("reads RateLimit-Reset from gmvRateLimit after RequestError strips headers", () => {
    const now = 1_700_000_000_000;
    const throttle = extractThrottle(
      {
        message: "Request Failed with status 429 - Too Many Requests",
          ...
          return {
            ...
            data: {
              gmvRateLimit: {
                status: 429,
                retryAfter: null,
                headers: { "x-ratelimit-reset": String(1_700_000_000 + 120) },
                headerNames: ["content-type", "x-ratelimit-reset"]
              }
            }
          };
        }
    ...
    assert.equal(throttle.waitMs, 120000);
    assert.deepEqual(throttle.headerNames, ["content-type", "x-ratelimit-reset"]);
  });
});
...
describe("formatThrottle", () => {
  ...
  it("marks a missing Retry-After on 429", () => {
    assert.match(formatThrottle({ status: 429 }), /retry-after=none/);
  });
});
