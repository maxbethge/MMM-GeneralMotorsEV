...
function parseDiagnostics(data) {
  ...
  Object.assign(out, parseAdvDiagnostics(data));
  return out;
}

function isAdvDiagnosticsAlert(color) {
  const c = upper(color);
  return Boolean(c) && c !== "GREEN";
}

function sanitizeRecommendedAction(raw) {
  const s = asString(raw);
  if (!s || /^(NA|N\/A|NONE|NULL|OK|NORMAL|NO ACTION|--)$/i.test(s)) {
    return null;
  }
  return s;
}

function collectAdvIssues(block) {
  const issues = [];
  const systems = Array.isArray(block?.diagnosticSystems) ? block.diagnosticSystems : [];
  for (const sys of systems) {
    if (!isAdvDiagnosticsAlert(sys?.systemStatusColor)) {
      continue;
    }
    const name = asString(sys.systemLabel || sys.systemShortDesc || sys.systemName);
    if (name) {
      issues.push(name);
    }
  }
...
function parseAdvDiagnostics(data) {
  if (!data || typeof data !== "object") {
    return {};
  }
  const block = data.advDiagnostics;
  if (block && typeof block === "object") {
    return compact({
      advDiagnosticsStatusColor: asString(block.advDiagnosticsStatusColor),
      advDiagnosticsStatus: asString(block.advDiagnosticsStatus),
      recommendedAction: sanitizeRecommendedAction(block.recommendedAction) || collectAdvIssues(block)
    });
  }
  return compact({
    advDiagnosticsStatusColor: asString(data.statusColor),
    advDiagnosticsStatus: asString(data.status),
    recommendedAction: sanitizeRecommendedAction(data.recommendedAction)
  });
}
...
function mergeState(parts) {
  const state = {
    ...
    displayName: null,
    imageUrl: null,
    advDiagnosticsStatusColor: null,
    advDiagnosticsStatus: null,
...
function keepPreviousValues(previous, next) {
  ...
  for (const [key, value] of Object.entries(previous)) {
    ...
    if (key === "advDiagnosticsStatusColor" || key === "advDiagnosticsStatus" || key === "recommendedAction") {
      continue;
    }
    if (key === "tires") {
  ...
  if (isEmptyValue(next.advDiagnosticsStatusColor)) {
    for (const key of ["advDiagnosticsStatusColor", "advDiagnosticsStatus", "recommendedAction"]) {
      if (isEmptyValue(out[key]) && !isEmptyValue(previous[key])) {
