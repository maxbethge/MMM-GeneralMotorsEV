...
const { payloadIsForInstance, wantsRangeDisplay, statusColorToCss } = require("./instance-payload");
...
describe("statusColorToCss", () => {
  it("maps GM status names to CSS colors", () => {
    assert.equal(statusColorToCss("YELLOW"), "yellow");
    assert.equal(statusColorToCss("red"), "red");
    assert.equal(statusColorToCss("#fc0"), "#fc0");
    assert.equal(statusColorToCss("not-a-color"), null);
  });
});
