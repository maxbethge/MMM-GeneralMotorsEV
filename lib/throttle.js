...
function extractThrottle(err, now = Date.now()) {
  ...
  const bag = err.gmvRateLimit || body.gmvRateLimit || null;
  const headers = {
    ...(bag?.headers || {}),
    ...(response.headers || {})
  };
  ...
  return {
    ...
    limit,
    remaining,
    headerNames: Array.isArray(bag?.headerNames) ? bag.headerNames : null,
    rateLimitHeaders: bag?.headers && Object.keys(bag.headers).length ? bag.headers : null
  };
}
