...
describe("isVehicleAsleep", () => {
  it("is not asleep when parked until a 429 has been seen", () => {
    assert.equal(isVehicleAsleep(parked), false);
  });

  it("is asleep when parked and a 429 has already been seen", () => {
    assert.equal(isVehicleAsleep(parked, { last429At: Date.now() }), true);
  });

  it("is awake when charging, plugged in, or ignition is on, even after a 429", () => {
    const last429At = Date.now();
    assert.equal(isVehicleAsleep({ ...parked, charging: true }, { last429At }), false);
    assert.equal(isVehicleAsleep({ ...parked, pluggedIn: true }, { last429At }), false);
    assert.equal(isVehicleAsleep({ ...parked, ignition: "ON" }, { last429At }), false);
  });

  it("is not asleep before any vehicle data exists", () => {
    assert.equal(isVehicleAsleep(null, { last429At: Date.now() }), false);
  });
});
...
describe("planPoll", () => {
  ...
  const last429At = now - 60 * 1000;

  it("fetches EV and diagnostics when parked after a 429, skips location and details when cached", () => {
    const plan = planPoll({ vehicle: parked, now, refreshMs, last429At });
  ...
  it("keeps parked polls awake until a 429 has been seen", () => {
    const plan = planPoll({
      vehicle: parked,
      now,
      refreshMs,
  ...
  it("does not wake the vehicle with forceRefreshEV while asleep", () => {
    const plan = planPoll({ vehicle: parked, now, refreshMs, forceEV: true, last429At });
    assert.equal(plan.forceEV, false);
    assert.ok(plan.skips.includes("forceEV:asleep"));
  });

  it("skips forceRefreshEV after the vehicle rejected live telemetry", () => {
    const plan = planPoll({
      vehicle: { ...parked, charging: true, pluggedIn: true },
      now,
      refreshMs,
      forceEV: true,
      forceEVUnsupported: true
    });
    assert.equal(plan.forceEV, false);
    assert.ok(plan.skips.includes("forceEV:unsupported"));
  });
  ...
  it("pings live location while asleep once the cached GPS is older than the asleep interval", () => {
    const plan = planPoll({
      vehicle: parked,
      now,
      refreshMs,
      last429At,
      lastLocationAt: now - 31 * 60 * 1000
    });
    assert.equal(plan.asleep, true);
    assert.equal(plan.location, true);
  });

  it("manual refresh fetches diagnostics, details, and a live location ping", () => {
    const plan = planPoll({
      vehicle: parked,
      now,
      refreshMs,
...
describe("pollDelayMs", () => {
  it("keeps the configured refresh interval while parked", () => {
