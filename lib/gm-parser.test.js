...
describe("gm-parser", () => {
  ...
  it("lets a live location ping replace cached EV GPS", () => {
    const state = parser.parseAll({
      evMetrics: evSample,
      location: {
        response: {
          data: { location: { lat: 42.33, lng: -83.05 } }
        }
      }
    });
    assert.equal(state.latitude, 42.33);
    assert.equal(state.longitude, -83.05);
    assert.equal(state.batteryLevel, 70);
  });
  ...
  it("reads recommendedAction when advDiagnosticsStatusColor is not GREEN", () => {
    const diag = parser.parseDiagnostics({
      diagnostics: [],
      advDiagnostics: {
        advDiagnosticsStatus: "CHECK",
        advDiagnosticsStatusColor: "YELLOW",
        recommendedAction: "See your dealer for service"
      }
    });
    assert.equal(diag.advDiagnosticsStatusColor, "YELLOW");
    assert.equal(diag.recommendedAction, "See your dealer for service");
    assert.equal(parser.isAdvDiagnosticsAlert(diag.advDiagnosticsStatusColor), true);
  });

  it("does not treat GREEN advanced diagnostics as an alert", () => {
    const diag = parser.parseDiagnostics({
      diagnostics: [],
      advDiagnostics: {
        advDiagnosticsStatus: "OK",
        advDiagnosticsStatusColor: "GREEN",
        recommendedAction: "No action"
      }
    });
    assert.equal(diag.advDiagnosticsStatusColor, "GREEN");
    assert.ok(!diag.recommendedAction);
    assert.equal(parser.isAdvDiagnosticsAlert(diag.advDiagnosticsStatusColor), false);
  });

  it("falls back to a non-green system name when recommendedAction is empty", () => {
    const diag = parser.parseDiagnostics({
      diagnostics: [],
      advDiagnostics: {
        advDiagnosticsStatus: "CHECK",
        advDiagnosticsStatusColor: "RED",
        recommendedAction: "",
        diagnosticSystems: [
          { systemName: "Engine", systemLabel: "Engine & Transmission", systemStatusColor: "RED" },
          { systemName: "OnStar", systemLabel: "OnStar", systemStatusColor: "GREEN" }
        ]
      }
    });
    assert.equal(diag.recommendedAction, "Engine & Transmission");
  });

  it("keeps a previous diagnostic alert when diagnostics are skipped", () => {
    const previous = {
      advDiagnosticsStatusColor: "YELLOW",
      recommendedAction: "Inflate left front tire",
      batteryLevel: 70
    };
    const next = parser.parseAll({ diagnostics: null, evMetrics: null });
    const kept = parser.keepPreviousValues(previous, next);
    assert.equal(kept.advDiagnosticsStatusColor, "YELLOW");
    assert.equal(kept.recommendedAction, "Inflate left front tire");
  });

  it("clears a previous diagnostic alert when the next poll is GREEN", () => {
    const previous = {
      advDiagnosticsStatusColor: "YELLOW",
      recommendedAction: "Inflate left front tire"
    };
    ...
    const kept = parser.keepPreviousValues(previous, next);
    assert.equal(kept.advDiagnosticsStatusColor, "GREEN");
    assert.equal(kept.recommendedAction, null);
  });
});
